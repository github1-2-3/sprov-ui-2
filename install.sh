...
install_sprov-ui() {
    ...
    last_version=$(curl --silent "https://api.github.com/repos/flydoos/sprov-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    echo -e "检测到sprov-ui最新版本：${last_version}，开始下载核心文件"
    wget -N --no-check-certificate -O /usr/local/sprov-ui/sprov-ui.jar https://github.com/flydoos/sprov-ui/releases/download/${last_version}/sprov-ui-${last_version}.jar
    if [[ $? -ne 0 ]]; then
    ...
    echo -e "wget -O /usr/bin/sprov-ui -N --no-check-certificate https://raw.githubusercontent.com/flydoos/sprov-ui/master/sprov-ui.sh && chmod +x /usr/bin/sprov-ui"
    echo -e ""
