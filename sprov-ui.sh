...
install() {
    install_base
    bash <(curl -L -s https://raw.githubusercontent.com/flydoos/sprov-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/sprov-ui -N --no-check-certificate https://raw.githubusercontent.com/flydoos/sprov-ui/master/sprov-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
